...
public class OrderController {
    ...
    public ResponseEntity<?> getOrderById(@PathVariable("id") UUID id, Principal principal) {
        if(principal == null)
    ...
    public ResponseEntity<?> addTickets(Principal principal, @RequestBody List<String> attractions) {
        if(principal == null)
            return ResponseFactory.createErrorResponse(HttpStatus.BAD_REQUEST, "Wrong or empty access token");
        else
            return orderService.addTickets(accountService.findUserByMail(principal.getName()), attractions);
    }
    ...
    public ResponseEntity<?> finishOrder(Principal principal,
                                         @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) Date visitdate) {
        if(principal == null)
            return ResponseFactory.createErrorResponse(HttpStatus.BAD_REQUEST, "Wrong or empty access token");
        else
